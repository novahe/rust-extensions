...
mod tests {
    ...
    fn test_create_piped_io() {
        ...
        let mut stdin_reader = io
            .stdin
            .as_ref()
            .map(|v| unsafe { File::from_raw_fd(libc::dup(v.rd)) });
        stdin_reader.as_mut().map(|v| v.read(&mut buf).unwrap());
        assert_eq!(&buf, &[0xfau8]);

        let mut stdout = io.stdout().unwrap();
        buf[0] = 0xce;
        let mut stdout_writer = io
            .stdout
            .as_ref()
            .map(|v| unsafe { File::from_raw_fd(libc::dup(v.wr)) });
        stdout_writer.as_mut().map(|v| v.write(&buf).unwrap());
        buf[0] = 0x0;
        ...
        buf[0] = 0xa5;
        let mut stderr_writer = io
            .stderr
            .as_ref()
            .map(|v| unsafe { File::from_raw_fd(libc::dup(v.wr)) });
        stderr_writer.as_mut().map(|v| v.write(&buf).unwrap());
        buf[0] = 0x0;
