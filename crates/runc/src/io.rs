...
mod tests {
    ...
    fn test_pipe_drop_closes_fds() {
        ...
        assert!(rd_res.is_err(), "rd_fd {} should be closed but fcntl returned {:?}", rd_fd, rd_res);
        assert!(wr_res.is_err(), "wr_fd {} should be closed but fcntl returned {:?}", wr_fd, wr_res);
    }
