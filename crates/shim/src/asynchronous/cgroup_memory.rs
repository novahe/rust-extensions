...
#[cfg(target_os = "linux")]
pub async fn monitor_oom(id: &String, pid: u32, tx: EventSender) -> Result<()> {
    if !is_cgroup2_unified_mode() {
        let path_from_cgorup = get_path_from_cgorup(pid).await?;
        let (mount_root, mount_point) = get_existing_cgroup_mem_path(path_from_cgorup).await?;

        let mem_cgroup_path = mount_point + &mount_root;
        let rx = register_memory_event(id, Path::new(&mem_cgroup_path), "memory.oom_control")
            .await
...
mod tests {
    ...
    async fn test_cgroupv1_oom_monitor() {
        if !is_cgroup2_unified_mode() {
            ...
            // Set oom monitor
            let path_from_cgorup = get_path_from_cgorup(pid).await.unwrap();
            let (mount_root, mount_point) = get_existing_cgroup_mem_path(path_from_cgorup)
                .await
                .unwrap();

            let mem_cgroup_path = mount_point + &mount_root;
            let mut rx = register_memory_event(
